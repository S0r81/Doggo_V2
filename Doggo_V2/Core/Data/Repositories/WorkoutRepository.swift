...
// Reads happen via @Query on the main context; this actor only owns the
// background-heavy work (CSV import) and id-based deletes. Returning or
// accepting a WorkoutSession would hand a background-context model across the
// boundary — the Swift 6 Sendable violation we deliberately avoid.
...
actor WorkoutRepository: WorkoutRepositoryProtocol {
    ...
    /// Deletes a workout session by id — value-in, so no model crosses the
    /// actor boundary.
    func delete(id: PersistentIdentifier) async throws {
